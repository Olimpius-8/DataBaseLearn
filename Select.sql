...
---optimize
select  Temp.ID,Name+' '+Lastname+' '+Surname as 'ФИО', counts as 'Количество найденных слов', rank() over (order by counts desc) as 'Ранг'
from (
select Coincedence.IDMajor as ID, sum(Coincedence.CountInMessage) as counts
from Coincedence
group by IDMajor
)as Temp
join ComradeMajor on Temp.ID = ComradeMajor.ID
group by TEmp.ID, counts, Name,Lastname,Surname
go
